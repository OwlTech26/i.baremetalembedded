...
t_error_code GPIO_port_config(const t_gpio_handle * const p_gpio_handle, const t_gpio_pin pin_sel)
{
	...
	if ((pin_sel < e_gpio_pin_max) &&
		...
		// Set Mode
		const uint32_t mode_sel = (p_gpio_handle->mode_sel > e_gpio_mode_analog) ? (uint32_t)e_gpio_mode_input : (uint32_t)p_gpio_handle->mode_sel;
		p_gpio_handle->p_gpio_reg->GPIOx_MODER &= GPIOx_MODER_NMASK(pin_sel);
		p_gpio_handle->p_gpio_reg->GPIOx_MODER |= (mode_sel << GPIOx_MODER_POS(pin_sel));
	...
	if ((p_gpio_handle->mode_sel > e_gpio_mode_analog) && (ret_stat == e_ec_no_error)) {
		const t_exti_it_mode it_mode_sel = (p_gpio_handle->mode_sel == e_gpio_mode_it_ft) ? e_exti_it_ft :
				(p_gpio_handle->mode_sel == e_gpio_mode_it_rt) ? e_exti_it_rt : e_exti_it_rft;
...
t_error_code GPIO_interrupt_config(const t_gpio_port port_sel, const t_gpio_pin pin_sel, const uint32_t pin_irq_prio)
{
	...
	if ((port_sel < e_gpio_max) &&
		...
		if (nvic_irq_idx < NUM_OF_ARRAY(nvic_exti_irq)) {
