...
t_task_register USER_TASK;
// t_task_register USER_TASK_DUMMY;

// uint8_t g_dummy = 0xDEADBEEFu;
...
static inline void debug_memory_dump(const uint32_t addr, const uint8_t length)
{
	...
	if (addr_end < SRAM_END_BASE) {
		for (uint32_t i=0u; i<length; ++i) {
			...
			DEBUG_PRINT("%lu [%#lX] = %#lX\n", i, curr_addr, data);
		}
	} else {
		DEBUG_PRINT(" -> [ERROR] Out of bounds '%#lX' from SRAM (%#X)\n", addr_end, SRAM_END_BASE);
	}
}
...
{
	while(1) {
		DEBUG_PRINT("LED 2 ON\n");
		task_delay(500u);
		DEBUG_PRINT("LED 2 OFF\n");
		task_delay(500u);
	}
}
...
{
	while(1) {
		DEBUG_PRINT("LED 3 ON\n");
		task_delay(250u);
		DEBUG_PRINT("LED 3 OFF\n");
		task_delay(250u);
	}
}
...
{
	while(1) {
		DEBUG_PRINT("LED 4 ON\n");
		task_delay(125u);
		DEBUG_PRINT("LED 4 OFF\n");
		task_delay(125u);
	}
}
...
int main(void)
{
	...
	DEBUG_PRINT(" --- Round-robin Scheduler for STM32 ---\n");
...
{
	DEBUG_PRINT("Exception: HardFault\n");
	while(1);
}

void MemManage_Handler(void)
{
	DEBUG_PRINT("Exception: MemManage\n");
	while(1);
}

void BusFault_Handler(void)
{
	DEBUG_PRINT("Exception: BusManage\n");
	while(1);
}
