...
int main(void)
{
	...
	*p_shcrs_reg |= ((1u << SHCRS_MEMFAULTENA) | (1u << SHCRS_BUSFAULTENA) | (1u << SHCRS_USGFAULTENA));
	...
	// 3. Enforce the processor to execute some undefined instruction
	uint32_t * const p_instr = (uint32_t * const)INSTR_SRAM_ADDRESS;
	*p_instr = 0xFFFFFFFFu; // Invalid instruction opcode
	void (*instr_fail)(void); // Function pointer for failing instruction
	instr_fail = (void *)((uint32_t)p_instr | 1u); // T-bit (LSB) shall always be odd to execute THUMB instruction instead of ARM.
	instr_fail(); // Invalid opcode shall be fetched from here
